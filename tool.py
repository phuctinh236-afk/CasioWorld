...
def extract_with_cloudscraper(short_url: str):
    """Fallback nhẹ khi không có Playwright (Termux / máy yếu)."""
    try:
        import cloudscraper
    except ImportError:
        return None, "cloudscraper chưa được cài"
    try:
        scraper = cloudscraper.create_scraper(
            browser={"browser": "chrome", "platform": "android", "mobile": True}
        )
        resp = scraper.get(short_url, timeout=30, allow_redirects=True)
        final_url = resp.url or ""
        if final_url and "link4m" not in final_url.lower():
            return final_url, None
        ...
        for pat in patterns:
            ...
            if matches:
                return matches[0], None
        return None, "Không tìm thấy link gốc (cloudscraper)"
    except Exception as e:
        return None, str(e)


def extract_final_url(short_url: str):
    # Thử Playwright trước (nếu có), không thì cloudscraper
    url, err = extract_with_playwright(short_url)
    if url:
        return url, None
    url2, err2 = extract_with_cloudscraper(short_url)
    if url2:
        return url2, None
    return None, err or err2 or "Không lấy được link"
