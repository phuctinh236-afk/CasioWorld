...
def deposit():
    ...
    if request.method == "POST":
        ...
        if action == "deposit":
            ...
            row2 = conn.execute(
                "SELECT COALESCE(total_deposit,0) as td, COALESCE(vip_level,1) as vl FROM users WHERE id = ?",
                (user["id"],)
            ).fetchone()
            ...
            conn.execute(
                """
                UPDATE users
                SET balance = ?, total_deposit = ?, vip_level = ?
                WHERE id = ?
                """,
                (new_balance, new_td, new_vip, user["id"])
            )
            conn.execute(
                """
            ...
            vip_msg = f" · VIP {new_vip}" if new_vip else ""
            user = get_current_user()
            return render_template(
                "deposit.html",
                user=user,
                success=f"Nạp thành công +{amount:,}. Số dư: {new_balance:,}{vip_msg}"
            )
...
def admin_api_users():
    ...
    for row in rows:
        ...
        users.append({
            ...
            "total_deposit": int(row["total_deposit"] or 0),
            "vip_level": int(row["vip_level"] or 1),
            "abnormal": (
