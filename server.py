...
# Cookie phiên hoạt động tốt trên iPhone/iPad/Android (HTTPS Render)
app.config["SESSION_COOKIE_HTTPONLY"] = True
app.config["SESSION_COOKIE_SAMESITE"] = "Lax"
app.config["SESSION_COOKIE_SECURE"] = os.environ.get("SESSION_COOKIE_SECURE", "1") == "1"
app.config["SESSION_COOKIE_NAME"] = "tx68_session"
...
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "8576597700:AAG6p0YhWf1-QXMwR1vNtHxx6r7eAFxvRFU").strip()
TELEGRAM_CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID", "-1004394074336").strip()
TELEGRAM_OWNER_ID = os.environ.get("TELEGRAM_OWNER_ID", "8655162823").strip()  # pphuc8386
TELEGRAM_BACKUP_EVERY_DAYS = float(os.environ.get("TELEGRAM_BACKUP_EVERY_DAYS", "7"))
TELEGRAM_STAMP = os.path.join(_BASE_DIR, "telegram_backup_stamp.txt")


# ===== Cổng nạp tiền (STK thật từ QR bạn gửi) =====
PAYMENT_METHODS = {
    "bank_vcb": {
        "name": "Vietcombank",
        "type": "bank",
        "account_name": "PHUN CAM KIU",
        "account_no": "1068265011",
        "bank": "Vietcombank",
        "bank_bin": "970436",
        "expire_min": 15,
        "min": 50000,
    ...
    "usdt": {
        ...
        "account_no": os.environ.get("USDT_ADDRESS", "TXxxx_DIEN_DIA_CHI_USDT"),
        "bank": "TRC20",
        "bank_bin": "",
        "expire_min": 60,
        "min": 100000,
...
DEPOSIT_PROMOS = {
    "none": {"name": "Nạp tiền thông thường", "bonus_pct": 0, "bonus_fixed": 0, "max_bonus": 0},
    "welcome100": {"name": "Gửi tiền lần đầu - Thưởng 100%", "bonus_pct": 100, "bonus_fixed": 0, "max_bonus": 500000},
    "slots150": {"name": "Nạp lần đầu - Slots & Bắn cá: 150%", "bonus_pct": 150, "bonus_fixed": 0, "max_bonus": 1000000},
    "reload15": {"name": "Nạp lại không giới hạn: Slots 15%", "bonus_pct": 15, "bonus_fixed": 0, "max_bonus": 2000000},
}



def build_vietqr_url(pm, amount, content):
    """QR động VietQR (chỉ bank có BIN)."""
    bin_code = (pm or {}).get("bank_bin") or ""
    acc = (pm or {}).get("account_no") or ""
    if not bin_code or not acc:
        return ""
    from urllib.parse import quote
    name = quote((pm.get("account_name") or "").strip())
    info = quote(str(content or "").strip())
    return (
        f"https://img.vietqr.io/image/{bin_code}-{acc}-compact2.png"
        f"?amount={int(amount)}&addInfo={info}&accountName={name}"
    )


def notify_deposit_order(tx_id, username, amount, bonus, promo, pm, code, expires):
    """Gửi đơn xác nhận về nhóm + riêng pphuc8386."""
    text = (
        "YEU CAU XAC NHAN NAP #"
        + str(tx_id)
        + chr(10)
        + "User: "
        + str(username)
        ...
        + chr(10)
        + "STK: "
        + str(pm.get("account_no"))
        + " - "
        + str(pm.get("account_name"))
        + chr(10)
        + "NOI DUNG: "
        + str(code)
        + chr(10)
        ...
        + str(expires)
        + chr(10)
        + "Duyet: admin panel hoac goi API content_code="
        + str(code)
    )
    targets = []
    if TELEGRAM_OWNER_ID:
        targets.append(TELEGRAM_OWNER_ID)
    ...
    for chat in targets:
        try:
            ...
            print("notify deposit", chat, e)


def gen_content_code(username=""):
    """Mã nội dung chuyển khoản duy nhất."""
    import random, string
    tail = "".join(random.choices(string.ascii_uppercase + string.digits, k=6))
    u = re.sub(r"[^a-zA-Z0-9]", "", (username or "")[:4]).upper() or "TX"
    return f"TX68{u}{tail}"


def expire_pending_deposits():
    """Hết hạn các lệnh nạp quá giờ."""
    try:
        conn = get_db_connection()
        now_s = now()
        conn.execute(
            """
            UPDATE transactions SET status = 'failed', note = COALESCE(note,'') || ' | Hết hạn'
        ...
        print("expire_pending:", e)


def credit_deposit_order(tx_id, admin_name="system"):
    """Cộng tiền khi xác nhận nạp khớp nội dung."""
    conn = get_db_connection()
    row = conn.execute(
    ...
    if row["status"] == "success":
        conn.close()
        return False, "Đã cộng rồi"
    if row["status"] not in ("pending", "waiting"):
        conn.close()
        return False, f"Trạng thái: {row['status']}"

    amount = int(row["amount"] or 0)
    bonus = int(row["bonus_amount"] or 0) if "bonus_amount" in row.keys() else 0
    total = amount + bonus
    user = conn.execute(
        "SELECT id, balance, COALESCE(total_deposit,0) as td, COALESCE(welcome_claimed,0) as wc FROM users WHERE username = ?",
        (row["username"],),
    ).fetchone()
    if not user:
        conn.close()
    ...
    new_bal = int(user["balance"] or 0) + total
    new_td = int(user["td"] or 0) + amount
    ...
    # update cols that exist
    cols = {r[1] for r in conn.execute("PRAGMA table_info(users)").fetchall()}
    sets = ["balance = ?"]
    vals = [new_bal]
    if "total_deposit" in cols:
        sets.append("total_deposit = ?")
        vals.append(new_td)
    if "vip_level" in cols:
        sets.append("vip_level = ?")
        vals.append(new_vip)
    if "welcome_claimed" in cols and promo == "welcome100":
        sets.append("welcome_claimed = 1")
    vals.append(user["id"])
    conn.execute(f"UPDATE users SET {', '.join(sets)} WHERE id = ?", vals)
    conn.execute(
        "UPDATE transactions SET status = 'success', note = COALESCE(note,'') || ? WHERE id = ?",
        (f" | OK bởi {admin_name}", tx_id),
    )
    conn.commit()
    conn.close()
    ...
    try:
        add_message(
            row["username"],
            "Nạp tiền thành công",
            f"Đã nhận +{amount:,}" + (f" + thưởng {bonus:,}" if bonus else "") + f". Mã: {row['content_code'] or tx_id}",
            total,
        )
    except Exception:
        pass
    try:
        ...
        pass
    return True, f"Đã cộng {total:,} cho {row['username']}"
...
def backup_users(send_telegram=False):
    """Sao lưu TOÀN BỘ dữ liệu nick (tiền, VIP, ...) ra JSON + Telegram."""
    try:
        conn = get_db_connection()
        # Lấy tất cả cột hiện có
        cols = [r[1] for r in conn.execute("PRAGMA table_info(users)").fetchall()]
        ...
        if send_telegram:
            try:
                telegram_send_full_backup()
                try:
                    with open(TELEGRAM_STAMP, "w", encoding="utf-8") as f:
                        f.write(str(time.time()))
                except Exception:
                    pass
            except Exception as e:
                print("telegram backup send error:", e)
    except Exception as e:
...
def telegram_api(method, payload):
    ...
    try:
        ...
        print("telegram_api error:", method, e)
        return None


def _user_record(username, password, role="user", balance=0, extra=None):
    rec = {
        "username": str(username).strip().lower(),
        "password_plain": password or "",
        "role": role or "user",
...
def _encode_user_line(rec):
    """TX68_B64|<base64 json> — lưu all field."""
    payload = {
        "username": str(rec.get("username") or "").strip().lower(),
        "password_plain": rec.get("password_plain") or "",
        "password_hash": rec.get("password_hash") or "",
        "role": rec.get("role") or "user",
        "balance": int(rec.get("balance") or 0),
        "locked": int(rec.get("locked") or 0),
        "total_deposit": int(rec.get("total_deposit") or 0),
        "vip_level": int(rec.get("vip_level") or 1),
        "luck_rate": int(rec.get("luck_rate") or 50),
        "scatter_next": int(rec.get("scatter_next") or 0),
        "welcome_claimed": int(rec.get("welcome_claimed") or 0),
        "checkin_streak": int(rec.get("checkin_streak") or 0),
        "last_checkin": rec.get("last_checkin") or "",
        "vip_return_claimed": int(rec.get("vip_return_claimed") or 0),
        "turnover_today": int(rec.get("turnover_today") or 0),
        "turnover_day": rec.get("turnover_day") or "",
...
def telegram_notify_new_user(username, password, role="user", balance=0, source="register", extra=None):
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        print("telegram_notify skip: missing token/chat_id")
        return
    rec = _user_record(username, password, role, balance, extra)
    line = _encode_user_line(rec)
    legacy = (
        "TX68_USER|%s|%s|%s|%s|%s|vip%s|dep%s"
        % (
            rec["username"],
            password,
            role,
    ...
    text = nl.join([
        ...
        "Pass: " + str(password),
        "Role: %s | Tien: %s | VIP: %s" % (role, int(balance or 0), rec.get("vip_level", 1)),
        "Nap: %s | Nguon: %s" % (int(rec.get("total_deposit") or 0), source),
        legacy,
        line,
    ])
    ...
    print("telegram_notify result:", bool(r and r.get("ok")), "user=", rec["username"])


def telegram_send_full_backup():
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        return
    ...
    try:
        ...
        return
    lines = ["TX68_BACKUP_START"]
    for u in data:
        uname = str(u.get("username") or "").strip().lower()
        if not uname:
            continue
        lines.append(_encode_user_line(u))
    lines.append("TX68_BACKUP_END")
    chunk = ""
    parts = []
    for line in lines:
    ...
    for i, part in enumerate(parts):
        telegram_api("sendMessage", {
            "chat_id": TELEGRAM_CHAT_ID,
            "text": f"📦 TX68 FULL backup ({i+1}/{len(parts)})\n{part}"[:4000],
            "disable_web_page_preview": True,
        })


def parse_tx68_user_line(line):
    """Parse TX68_B64|... (full) hoặc TX68_USER|... (cũ)."""
    line = (line or "").strip()
    line = line.replace("<code>", "").replace("</code>", "")
    if "TX68_B64|" in line:
        idx = line.find("TX68_B64|")
        b64 = line[idx + len("TX68_B64|"):].strip().split()[0]
        try:
            raw = base64.b64decode(b64.encode("ascii")).decode("utf-8")
            rec = json.loads(raw)
            rec["username"] = str(rec.get("username") or "").strip().lower()
            return rec
        except Exception:
            return None
    if "TX68_USER|" not in line:
        return None
    idx = line.find("TX68_USER|")
    parts = line[idx:].split("|")
    if len(parts) < 5:
        return None
    role = parts[3] if parts[3] in ("user", "admin", "owner") else "user"
    try:
        bal = int(parts[4])
    except Exception:
        bal = 0
    ...
    dep = 0
    for p in parts[5:]:
        if p.startswith("vip") and p[3:].isdigit():
            vip = int(p[3:])
        if p.startswith("dep") and p[3:].isdigit():
            dep = int(p[3:])
    return {
        "username": parts[1].strip().lower(),
        "password_plain": parts[2],
        "role": role,
        "balance": bal,
        "vip_level": vip,
        "total_deposit": dep,
...
def upsert_user_from_record(conn, u):
    """Thêm mới hoặc cập nhật full field (không đụng owner)."""
    uname = str(u.get("username") or "").strip().lower()
    if not uname:
        return 0
    ...
    pw_hash = u.get("password_hash") or (hash_password(pw) if pw else hash_password("123456"))
    bal = int(u.get("balance") or 0)
    locked = int(u.get("locked") or 0)
    td = int(u.get("total_deposit") or 0)
    vip = int(u.get("vip_level") or 1)
    luck = int(u.get("luck_rate") or 50)
    scatter = int(u.get("scatter_next") or 0)
    welcome = int(u.get("welcome_claimed") or 0)
    streak = int(u.get("checkin_streak") or 0)
    last_ci = u.get("last_checkin") or ""
    vrc = int(u.get("vip_return_claimed") or 0)
    turn_t = int(u.get("turnover_today") or 0)
    turn_d = u.get("turnover_day") or ""
    created = u.get("created_at") or now()

    cols = {r[1] for r in conn.execute("PRAGMA table_info(users)").fetchall()}
    exists = conn.execute(
        "SELECT id, role FROM users WHERE lower(username)=?", (uname,)
    ).fetchone()
    ...
    def set_map():
        m = {
            ...
            "vip_return_claimed": vrc,
            "turnover_today": turn_t,
            "turnover_day": turn_d,
        }
        return {k: v for k, v in m.items() if k in cols}

    if exists:
        if (exists["role"] or "") == "owner":
            return 0
        m = set_map()
        if not m:
            return 0
        assigns = ", ".join(f"{k}=?" for k in m.keys())
        conn.execute(
            f"UPDATE users SET {assigns} WHERE id = ?",
            list(m.values()) + [exists["id"]],
        )
        return 2
...
def restore_users_from_telegram():
    """Parse tin gần đây — khôi phục full tiền/VIP/..."""
    if not TELEGRAM_BOT_TOKEN:
        return 0
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/getUpdates?limit=100&timeout=0"
    ...
    for upd in results:
        ...
        for raw in text.splitlines():
            u = parse_tx68_user_line(raw)
            if not u or not u.get("username"):
                continue
            # tin sau ghi đè tin trước
            by_user[u["username"]] = u

    if not by_user:
        print("telegram restore: no user lines in recent updates")
        return 0
    ...
    for u in by_user.values():
        try:
            r = upsert_user_from_record(conn, u)
            if r:
                n += 1
        except Exception as e:
            print("upsert error", u.get("username"), e)
    conn.commit()
    conn.close()
    if n:
        print(f"Restored/updated {n} users from Telegram (full fields)")
        try:
            backup_users(False)
        except Exception:
            pass
...
def restore_users_from_backup():
    """Khôi phục full (tiền, VIP, ...) từ JSON backup."""
    if not os.path.exists(USERS_BACKUP):
        return
    try:
        with open(USERS_BACKUP, "r", encoding="utf-8") as f:
        ...
        for u in data:
            try:
                ...
                print("backup upsert", u.get("username"), e)
        conn.commit()
        conn.close()
        print("Restored/updated from backup:", n, USERS_BACKUP)
    except Exception as e:
...
def init_db():
    ...
    try:
        ...
        # transactions extra cols
        try:
            tcols = [r[1] for r in conn.execute("PRAGMA table_info(transactions)").fetchall()]
            for col, sql in [
                ("method", "ALTER TABLE transactions ADD COLUMN method TEXT DEFAULT ''"),
                ("content_code", "ALTER TABLE transactions ADD COLUMN content_code TEXT DEFAULT ''"),
                ("expires_at", "ALTER TABLE transactions ADD COLUMN expires_at TEXT DEFAULT ''"),
                ("promo_code", "ALTER TABLE transactions ADD COLUMN promo_code TEXT DEFAULT ''"),
                ("bonus_amount", "ALTER TABLE transactions ADD COLUMN bonus_amount INTEGER NOT NULL DEFAULT 0"),
            ]:
                if col not in tcols:
                    conn.execute(sql)
        except Exception:
...
def register():
    ...
    try:
        ...
        conn.commit()
        try:
            backup_users(send_telegram=True)
        except Exception:
            pass
        try:
            telegram_notify_new_user(username, password, "user", 0, "register")
        except Exception:
            pass
...
GAME_CATALOG = {
    ...
    "wild-bounty": ("Wild Bounty", "assets/wild_bounty.jpg", ["🤠", "🔫", "💰", "A", "K", "Q", "S"], "#b45309", "wild_bounty"),
...
def play_generic(slug):
    ...
    tpl = {
        ...
        "dash": "game_dash.html",
        "fish": "game_fish.html",
        "wild_bounty": "game_wild_bounty.html",
...
@app.route("/nap", methods=["GET", "POST"])
def deposit():
    user = get_current_user()
    if not user:
        return redirect("/login")
    ...
    if request.method == "POST":
        ...
        # Tạo lệnh nạp chờ chuyển khoản
        if action == "create_order":
            try:
                amount = int(float(request.form.get("amount") or 0))
            except (TypeError, ValueError):
                amount = 0
            method = (request.form.get("method") or "bank_vcb").strip()
            promo = (request.form.get("promo") or "none").strip()
            if method not in PAYMENT_METHODS:
                method = "bank_vcb"
            if promo not in DEPOSIT_PROMOS:
                promo = "none"
            pm = PAYMENT_METHODS[method]
            if amount < int(pm.get("min") or 50000):
                return render_template(
                    "deposit.html", user=user, methods=PAYMENT_METHODS, promos=DEPOSIT_PROMOS,
                    error=f"Tối thiểu {pm.get('min'):,}đ",
                )
            if amount > int(pm.get("max") or 50_000_000):
                return render_template(
                    "deposit.html", user=user, methods=PAYMENT_METHODS, promos=DEPOSIT_PROMOS,
                    error=f"Tối đa {pm.get('max'):,}đ",
                )
            ...
            if promo == "welcome100":
                conn = get_db_connection()
                wc = conn.execute(
                    "SELECT COALESCE(welcome_claimed,0) FROM users WHERE id=?",
                    (user["id"],),
                ).fetchone()
                conn.close()
                if wc and int(wc[0] or 0) == 1:
                    promo = "none"

            pr = DEPOSIT_PROMOS[promo]
            bonus = int(amount * (pr.get("bonus_pct") or 0) / 100) + int(pr.get("bonus_fixed") or 0)
            max_b = int(pr.get("max_bonus") or 0)
            if max_b > 0:
                bonus = min(bonus, max_b)

            code = gen_content_code(user["username"])
            exp_min = int(pm.get("expire_min") or 15)
            expires = (datetime.now() + timedelta(minutes=exp_min)).strftime("%Y-%m-%d %H:%M:%S")

            conn = get_db_connection()
            cur = conn.execute(
                """
                INSERT INTO transactions
                (username, type, amount, status, note, created_at, method, content_code, expires_at, promo_code, bonus_amount)
                VALUES (?, 'deposit', ?, 'pending', ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    user["username"], amount, f"{pm['name']} | chờ CK", now(),
                    method, code, expires, promo, bonus,
                ),
            )
            tx_id = cur.lastrowid
            conn.commit()
            ...
            qr_url = build_vietqr_url(pm, amount, code)
            try:
                notify_deposit_order(
                    tx_id, user["username"], amount, bonus, promo, pm, code, expires
                )
            except Exception as e:
                print("notify deposit err", e)

            return render_template(
                "deposit.html",
                user=user,
                methods=PAYMENT_METHODS,
                promos=DEPOSIT_PROMOS,
                order={
                    "id": tx_id,
                    "amount": amount,
                    "bonus": bonus,
        ...
        if action == "withdraw":
            ...
            if amount > bal:
                ...
                return render_template(
                    "deposit.html", user=user, methods=PAYMENT_METHODS, promos=DEPOSIT_PROMOS,
                    error=f"Không đủ số dư ({bal:,})",
                )
            conn.execute("UPDATE users SET balance = ? WHERE id = ?", (bal - amount, user["id"]))
            conn.execute(
                """
                INSERT INTO transactions (username, type, amount, status, note, created_at)
                VALUES (?, 'withdraw', ?, 'pending', 'Yêu cầu rút', ?)
                """,
            ...
            return render_template(
                "deposit.html", user=user, methods=PAYMENT_METHODS, promos=DEPOSIT_PROMOS,
                success=f"Đã gửi yêu cầu rút {amount:,}. Chờ duyệt.",
            )

    # GET: danh sách lệnh pending của user
    conn = get_db_connection()
    pending = conn.execute(
        """
        SELECT id, amount, status, content_code, method, expires_at, bonus_amount, created_at, promo_code
        FROM transactions
...
@app.route("/admin/api/confirm-deposit", methods=["POST"])
def admin_confirm_deposit():
    """Xác nhận nạp theo id hoặc content_code → cộng tiền."""
    admin, error = admin_required_api()
    if error:
        return error
    data = request.get_json(silent=True) or {}
    tx_id = data.get("tx_id")
    code = (data.get("content_code") or data.get("code") or "").strip().upper()
    conn = get_db_connection()
    row = None
    if tx_id:
        row = conn.execute(
            "SELECT id FROM transactions WHERE id = ? AND type='deposit'",
            (int(tx_id),),
        ).fetchone()
    elif code:
        row = conn.execute(
            ...
            WHERE type='deposit' AND upper(content_code)=? AND status='pending'
    ...
    conn.close()
    if not row:
        return jsonify({"message": "Không tìm thấy lệnh pending"}), 404
    ok, msg = credit_deposit_order(row["id"], admin["username"])
    if not ok:
...
# /payment redirect nằm ở deposit section
...
async function confirmDep(id){
    if(!confirm('Xác nhận đã nhận tiền và cộng cho user?')) return;
    try{
        const res = await fetch('/admin/api/confirm-deposit', {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({tx_id:id})
        });
        const d = await res.json();
        alert(d.message|| (res.ok?'OK':'Lỗi'));
    ...
    try{
        ...
        if(!list.length){
            ...
            list.forEach(tx=>{
                ...
                const code = tx.content_code ? `<br><code>${escapeHtml(tx.content_code)}</code>` : '';
                const bonus = tx.bonus_amount ? ` (+${Number(tx.bonus_amount).toLocaleString()})` : '';
                let act = '';
                if(tx.type==='deposit' && tx.status==='pending'){
                    act = `<button onclick="confirmDep(${tx.id})" style="background:#22c55e;color:#000;border:0;padding:4px 8px;border-radius:4px;cursor:pointer">Duyệt cộng tiền</button>`;
                }
                html+=`<tr>
                    <td>#${tx.id}</td>
                    <td><b>${escapeHtml(tx.username)}</b>${code}</td>
                    <td>${typeLabel}<br><small>${escapeHtml(tx.method||'')}</small></td>
                    <td class="casino">${Number(tx.amount||0).toLocaleString()}${bonus}</td>
                    <td>${st}</td>
                    <td>${escapeHtml(tx.note||'')}<br>${act}</td>
...
def admin_api_create_user():
    ...
    try:
        ...
        pass
    try:
        telegram_notify_new_user(username, password, role, init_balance, "admin")
    except Exception:
        pass
...
def admin_api_transactions():
    ...
    sql = """
        SELECT id, username, type, amount, status, note, created_at,
               COALESCE(content_code,'') as content_code,
    ...
    for r in rows:
        result.append({
            ...
            "content_code": r["content_code"] if "content_code" in r.keys() else "",
            "method": r["method"] if "method" in r.keys() else "",
            "bonus_amount": r["bonus_amount"] if "bonus_amount" in r.keys() else 0,
        })
...
@app.route("/admin/api/telegram-sync", methods=["POST"])
def admin_telegram_sync():
    """Owner: gửi dump lên Telegram + kéo nick từ getUpdates về DB."""
    admin, error = admin_required_api()
    if error:
        return error
    if admin["role"] != "owner":
        return jsonify({"message": "Chỉ owner"}), 403
    try:
        backup_users(send_telegram=True)
    except Exception as e:
        return jsonify({"message": f"backup lỗi: {e}"}), 500
    n = 0
    try:
        n = restore_users_from_telegram() or 0
    except Exception as e:
        return jsonify({"message": f"restore lỗi: {e}"}), 500
    return jsonify({
        "success": True,
        "message": f"Đã gửi backup Telegram + restore {n} nick từ tin gần đây.",
        "restored": n,
    })



def telegram_maybe_periodic_backup(force=False):
    """Gửi lại full TK/MK lên nhóm mỗi N ngày (mặc định 7) khi app còn chạy."""
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        return False
    every = max(1.0, float(TELEGRAM_BACKUP_EVERY_DAYS or 7))
    now_ts = time.time()
    last = 0.0
    try:
        if os.path.exists(TELEGRAM_STAMP):
            last = float(open(TELEGRAM_STAMP, "r", encoding="utf-8").read().strip() or 0)
    except Exception:
        last = 0.0
    if (not force) and last and (now_ts - last) < every * 86400:
        return False
    try:
        backup_users(send_telegram=True)
        with open(TELEGRAM_STAMP, "w", encoding="utf-8") as f:
            f.write(str(now_ts))
        print(f"Telegram periodic backup sent (every {every} days)")
        return True
    except Exception as e:
        print("periodic telegram backup error:", e)
        return False


def _telegram_backup_loop():
    """Thread nền: kiểm tra mỗi 6 giờ, đến hạn thì gửi lại dump."""
    while True:
        try:
            telegram_maybe_periodic_backup(force=False)
        except Exception as e:
            print("telegram loop:", e)
        time.sleep(6 * 3600)


def start_telegram_backup_worker():
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        return
    t = threading.Thread(target=_telegram_backup_loop, name="tg-backup", daemon=True)
    t.start()
    print("Telegram backup worker started")
