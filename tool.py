...
def extract_final_url(short_url: str):
    """Cố gắng vượt Cloudflare bằng cloudscraper + fallback requests"""
    headers = {
        ...
        "Referer": "https://link4m.net/",
    }

    # Thử cloudscraper trước
    try:
        import cloudscraper
        scraper = cloudscraper.create_scraper(
            browser={'browser': 'chrome', 'platform': 'android', 'mobile': True}
        )
        resp = scraper.get(short_url, headers=headers, timeout=25, allow_redirects=True)
        final = resp.url
        if "link4m" not in final.lower() and resp.status_code == 200:
            return final

        content = resp.text
        # Tìm link đích trong HTML
        patterns = [
            ...
            r'href=["\'](https?://(?!link4m|cloudflare|google|facebook)[^"\']+)["\']',
        ]
        for pat in patterns:
    ...
    try:
        ...
        resp = session.get(short_url, headers=headers, timeout=20, allow_redirects=True)
        final = resp.url
...
def bypass():
    ...
    return jsonify({"success": False, "message": "Không lấy được link gốc (Cloudflare quá mạnh)"})
