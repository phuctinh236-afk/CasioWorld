...
<style>
body{margin:0;background:#0b1120;color:#fff;font-family:Arial}
.box{max-width:500px;margin:30px auto;padding:20px;
background:#131c2e;border:1px solid #263653;border-radius:12px;text-align:center}
button{padding:12px 25px;border:0;border-radius:8px;
background:#f3a838;font-weight:bold;cursor:pointer}
.result{font-size:22px;margin:20px}
</style>
</head>
<body>
<div class="box">
<h1>🎯 Điểm số</h1>
<p>FC: <b id="balance">{{ user["balance"] }}</b></p>
<input id="amount" type="number" min="1" value="10"
style="padding:10px;width:100px">
<br><br>
<button onclick="play()">CHƠI</button>
...
async function play(){
    ...
    if(!amount || amount<=0){
        result.textContent="Nhập số FC hợp lệ";
        return;
    }
    ...
    result.textContent=data.message || "Hoàn tất";
...
def game_play():
    ...
    # Giới hạn mỗi lượt.
    if amount > 100000:
        return jsonify({"message": "Tối đa 100.000 FC mỗi lượt"}), 400
    ...
    if not current or current["balance"] < amount:
        conn.close()
        return jsonify({"message": "Không đủ FC"}), 400

    # Game demo cố định: mỗi lượt trừ số FC chơi.
    # Đây chỉ là điểm ảo, không có giá trị tiền.
    ...
    return jsonify({
        "success": True,
        "message": f"Đã sử dụng {amount:,} FC ảo.",
        "balance": new_balance
    })
...
if __name__ == "__main__":
    ...
    print("Owner mặc định: owner / owner123")
