...
def chat():
    ...
    try:
        learned = get_ai().learn_from_input(msg)
        reply = learned if learned else get_ai().respond(msg)
    except Exception as e:
        reply = "Lỗi AI: " + str(e)
...
<div id="main">
  ...
  <div id="input-area">
    ...
    <div class="hint" id="quota-hint">Bấm ☰ để mở menu · Có thể chat ngay</div>
  </div>
</div>
...
async function openModal(kind) {
  ...
  if (kind === 'projects') {
    modalBox.innerHTML = '<h2>📁 Dự án</h2><p>Tính năng đang phát triển.</p><button class="modal-close" onclick="closeModal()">Đóng</button>';
  } else {
    modalBox.innerHTML = '<h2>AI VIP Premium</h2><p>Liên hệ admin để nâng VIP.</p><button class="modal-close" onclick="closeModal()">Đóng</button>';
    try {
      const res = await fetch('/premium-info');
      const data = await res.json();
      const items = (data.benefits || []).map(b => '<li>' + b + '</li>').join('');
      modalBox.innerHTML = '<h2>AI VIP Premium</h2><ul>' + items + '</ul><button class="modal-close" onclick="closeModal()">Đóng</button>';
    } catch (e) {}
  }
...
function clearChat() {
  if (!chatBox) return;
  chatBox.innerHTML = '';
  addBubble('Chào bạn! Hỏi mình về lập trình, hoặc dạy mình: học|câu hỏi|câu trả lời', 'ai');
}

function renderUserBox() {
  if (!sidebarUserBox) return;
  if (!ME || !ME.name) {
    sidebarUserBox.innerHTML = '<a class="login-btn" href="/login/google">Đăng nhập Google</a>';
    if (quotaHint) quotaHint.textContent = 'Chưa đăng nhập · vẫn chat được';
    return;
  }
  ...
  sidebarUserBox.innerHTML = avatarHtml +
    '<div class="user-info"><div class="line1"><span class="' + nameClass + '">' + ME.name + '</span>' + vipIcon +
    '</div><span class="email">' + (ME.email || '') + '</span></div>' +
    '<a href="/logout" class="logout-btn" title="Đăng xuất">⏻</a>';
  if (quotaHint) {
    quotaHint.textContent = ME.is_vip
      ? 'VIP: ảnh/file không giới hạn'
      : ('Ảnh: ' + (ME.image_count || 0) + '/' + (ME.image_limit || 3) + ' · File: ' + (ME.file_count || 0) + '/' + (ME.file_limit || 3));
  }
}
...
async function loadMe() {
  try {
    ...
    ME = (data && data.name) ? data : null;
    renderUserBox();
  } catch (err) {
    ME = null;
    renderUserBox();
    if (quotaHint) quotaHint.textContent = 'Chế độ khách · bấm ☰ mở menu';
  }
}
...
async function loadConversations() {
  ...
  try {
    ...
    list.forEach(function(c) {
      ...
      item.innerHTML = '<span style="overflow:hidden;text-overflow:ellipsis;">' + (c.title || 'Chat') +
        '</span><button class="del-btn" type="button">🗑</button>';
    ...
    convList.innerHTML = '<div class="conv-empty">Không tải lịch sử</div>';
  }
}
...
async function openConversation(id) {
  ...
  try {
    ...
    addBubble('Không mở được hội thoại.', 'ai');
  }
  closeSidebar();
}
...
async function sendMessage() {
  ...
  try {
    ...
    const response = await fetch('/chat', {
      ...
      body: JSON.stringify({ message: message, conversation_id: CURRENT_CONV_ID }),
      signal: ctrl.signal
    });
    clearTimeout(timer);
    const data = await response.json().catch(function() { return {}; });
    if (!response.ok) {
      addBubble(data.error || data.message || ('Lỗi server ' + response.status), 'ai');
    } else {
      addBubble(data.reply || '(không có trả lời)', 'ai');
      if (data.conversation_id) CURRENT_CONV_ID = data.conversation_id;
    }
  } catch (e) {
    addBubble('Không gửi được. Thử lại sau. (' + (e.name || e.message || e) + ')', 'ai');
  } finally {
    sending = false;
...
async function uploadPickedFile() {
  ...
  try {
    const res = await fetch(isImage ? '/upload/image' : '/upload/file', { method: 'POST', body: form });
    const data = await res.json().catch(function() { return {}; });
    if (data.error) addBubble('⚠️ ' + data.error, 'ai');
    else { addBubble('Đã gửi: ' + file.name, 'user'); loadMe(); }
  } catch (e) {
    addBubble('Upload lỗi.', 'ai');
  }
  fileInput.value = '';
}
...
document.querySelectorAll('.hamburger').forEach(function(btn) {
  btn.addEventListener('click', function(e) {
    e.preventDefault();
    e.stopPropagation();
    if (btn.closest && btn.closest('#sidebar')) closeSidebar(e);
    else openSidebar(e);
  });
});
