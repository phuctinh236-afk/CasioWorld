...
# Map slug -> (tên hiển thị, ảnh)
# name, image, theme symbols (emoji list), accent color
GAME_CATALOG = {
    "dragon-gems": ("Dragon Gems 500", "assets/dragon_gems.jpg", ["💎", "🐉", "🔴", "🟡", "🟢", "🔵", "7️⃣"], "#e11d48"),
    "mithai-madness": ("Mithai Madness", "assets/mithai_madness.jpg", ["🍬", "🍭", "🧁", "🍩", "🍪", "🎂", "⭐"], "#f97316"),
    "fortune-ganesha": ("Fortune Ganesha", "assets/fortune_garuda.jpg", ["🐘", "🪷", "💰", "🪙", "🪔", "🕉️", "⭐"], "#eab308"),
    "ncvip-super": ("NCVIP Super Elements", "assets/ncvip_super.jpg", ["🔥", "💧", "🌪️", "🪨", "⚡", "🌙", "☀️"], "#06b6d4"),
    "ultra-ace": ("Ultra Ace", "assets/ultra_ace.jpg", ["🃏", "♠️", "♥️", "♦️", "♣️", "👑", "⭐"], "#a855f7"),
    "ncvip-gates": ("Gates of Olympus", "assets/ncvip_gates.jpg", ["⚡", "🏛️", "👑", "💎", "🔵", "🟣", "🟡"], "#f59e0b"),
    "bountiful-birds": ("NC Bountiful Birds", "assets/bountiful_birds.jpg", ["🦜", "🦅", "🪶", "🪺", "🐦", "🌴", "⭐"], "#22c55e"),
    "treasures-aztec": ("Treasures of Aztec", "assets/treasures_aztec.jpg", ["🗿", "🌞", "💎", "🐍", "🏺", "🌽", "⭐"], "#b45309"),
    "wild-bounty": ("Wild Bounty", "assets/wild_bounty.jpg", ["🤠", "🐎", "🔫", "💰", "🥃", "⭐", "🌵"], "#ca8a04"),
    "dragon-gems-wheel": ("Dragon Gems Wheel", "assets/dragon_gems_wheel.jpg", ["🎡", "🐉", "💎", "🔴", "🟡", "🟢", "⭐"], "#dc2626"),
    "pinata-wins": ("Pinata Wins", "assets/pinata_wins.jpg", ["🪅", "🎉", "🍬", "🎈", "🌮", "🌶️", "⭐"], "#ec4899"),
    "jackpot-fishing": ("Jackpot Fishing", "assets/jackpot_fishing.jpg", ["🐟", "🦈", "🐙", "🐚", "⚓", "💎", "⭐"], "#0ea5e9"),
    "super-niubi": ("Super Niu Niu", "assets/super_niubi.jpg", ["🐂", "🥩", "🔔", "💰", "🪙", "🍀", "⭐"], "#ef4444"),
    "lucky-neko": ("Lucky Neko", "assets/lucky_neko.jpg", ["🐱", "🍣", "💴", "🎏", "🌸", "💎", "⭐"], "#f472b6"),
    "chinese-new-year": ("Chinese New Year", "assets/chinese_new_year.jpg", ["🧧", "🐉", "🏮", "🥟", "🪙", "🎆", "⭐"], "#dc2626"),
    "chicken-drop": ("Chicken Drop", "assets/chicken_dash.jpg", ["🐔", "🥚", "🌽", "🌾", "🪺", "⭐", "💰"], "#facc15"),
    "daga": ("Đá Gà", "assets/daga_ws151.jpg", ["🐓", "🏆", "💰", "🔥", "⭐", "🥊", "💵"], "#b91c1c"),
}
...
def play_generic(slug):
    ...
    if not info:
        ...
        game_name, img_path, symbols, accent = info
    ...
    if request.method == "POST":
        ...
        if action == "deposit":
            ...
            conn.execute(
                ...
                VALUES (?, 'deposit', ?, 'success', 'Nạp tự động', ?)
                """,
    ...
    if(!users.length){
        ...
        users.forEach(u=>{
            ...
            <tr style="${rowStyle}">
                ...
                <td>${roleBadge}</td>
                <td>${statusTxt}</td>
                <td style="font-size:12px">🍀${u.luck_rate||50}% ✨${u.scatter_next||0}</td>
                <td>
...
def admin_api_users():
    ...
    # Người bất thường: số dư âm, hoặc số dư quá lớn, hoặc khóa
    abnormal = conn.execute("""
        SELECT COUNT(*) FROM users
        WHERE COALESCE(balance,0) < 0
           OR COALESCE(balance,0) > 50000000
           OR COALESCE(locked,0) = 1
    """).fetchone()[0]
    ...
    for row in rows:
        ...
        users.append({
            ...
            "password": pw,
            "luck_rate": int(row["luck_rate"] or 50),
            "scatter_next": int(row["scatter_next"] or 0),
            "abnormal": (
                int(row["balance"] or 0) < 0
                or int(row["balance"] or 0) > 50_000_000
                or int(row["locked"] or 0) == 1
            )
        })
...
def admin_api_create_user():
    ...
    try:
        init_balance = int(data.get("balance") or 1000)
        if init_balance < 0:
            init_balance = 0
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, 0, ?)
        """,
        (username, hash_password(password), password, role, init_balance, now())
    )
    conn.commit()
    conn.close()
    ...
    return jsonify({
        "success": True,
        "message": f"Đã tạo {role}: {username} (số dư {init_balance:,})."
    })
